// EDC16 VAG ECU detector - Universal Algorithm Based on reference analysis
...
//
// Detection Strategy (inspired by reference analysis):
...
/// All EDC16 map templates based on reference analysis
...
impl EDC16U34Detector {
    ...
    pub fn detect(&self, data: &[u8]) -> Vec<DetectedMap> {
            ...
            if !self.overlaps(&detected_ranges, range) {
                ...
                for map in duration_maps {
                    ...
                    if !self.overlaps(&detected_ranges, map_range) {
                        ...
                        // request… SAUF la première dynamique 10x10 : la référence
                ...
                // durations (réf. externe : 0x1E5DC8 Golf 2.0, 0x1DFC76
        ...
        // ========== PHASE 0.65: extras de parité avec la référence ==========
        ...
        // no the reference tools counterpart (false-positive flood flagged
               ...
               data[offset + 4] == 0x00 && data[offset + 5] == 0x00 {
                ...
                if pedal_end + 2 <= data.len() &&
                   data[pedal_end] == 0x27 && data[pedal_end + 1] == 0x10 {
                    ...
                    // (la référence liste « Driver Wish 08 » à zéro, 0x1C2CFE) —
                    ...
                    if all_zeros {
                        ...
                        // que la référence liste après le cluster (0x1C36CE).
    ...
    // Valid configurations (from reference analysis):
    ...
    fn detect_n75_by_signature(&self, data: &[u8], detected: &HashSet<u32>) -> Vec<DetectedMap> {
        ...
        // (rows, cols) pairs
        // the reference tool only ever lists the PCR_rBPCtlBas/Bas2 pairs (16 RPM rows
        ...
        // the generic RPM validator skips it. Listed by the reference tool on every
                    ...
                    if potential_cols == SOI_COLS as u16 {
                        ...
                        if self.is_valid_soi_iq_axis(&iq_axis) {
                            ...
                            if valid_block {
                                ...
                                // The reference tool shows dynamic maps BEFORE the 10 main SOI maps
                                // These are marked with '*' in the reference tool
                                // IMPORTANT: The dynamic maps may not be directly adjacent to the main block
                                // The reference tool shows 4 dynamic maps at addresses like:
                                ...
                                // The reference tool shows dynamic maps that may overlap with the main block by up to 256 bytes
    ...
    fn detect_torque_limiter_maps(
        ...
        if let Some(first) = all_candidates.first().cloned() {
            ...
            // La COPIE de 2e banque (la référence la liste : 0x18A624 + 0x1D172C
    ...
    fn detect_launch_control_maps(
        ...
        let mut maps = Vec::new();
        // Dimensions: 10x10 as used by tuning software
    ...
    /// Score a potential map based on multiple factors (reference style)
    ...
    fn detect_duration_block_maps(
        ...
        // the block (previously produced spurious "Duration 08/09" that the reference tool
        // does not list).
    ...
    /// Réf. externe : facteur 0.023427 deg CrS, données quasi plates 200-3500.
    ...
    /// Extras de parité avec la référence (réfs des 24 fichiers du banc) :
    ...
    fn detect_reference_extras(&self, data: &[u8], detected: &HashSet<u32>) -> Vec<DetectedMap> {
        ...
        while i < scan_hi {
            ...
            if let Some((ey0, ex0, ed0)) = eft {
                ...
                // la référence liste la paire dans les DEUX banques (0x18Dxxx et
    ...
    /// EDC16U34: Based on reference patterns analysis (lectureG5.bin)
    /// Reference addresses: 1EFFB4, 1EFFBE, 1EFFC0, 1F33CA, 1F33D2, 1F3410
    ...
    /// Verified on abcABC (reference): 0 false positives in 0x1E0000-0x1E8000.
    #[allow(dead_code)]
    ...
    pub fn detect_maximum_vehicle_speed(&self, data: &[u8], detected_addresses: &HashSet<u32>) -> Vec<DetectedMap> {
        ...
        // Reference: 1EFFB4 (61A8=250), 1EFFBE (61A8=250), 1EFFC0 (7530=300)
        ...
        // variant: late variants put it around 0x1F33xx (reference 1F33CA, 1F33D2),
        ...
        // which the reference tool does not list). With the strict check, exactly ONE match
        ...
        // Reference (abcABC/ZZZori firmware): 1E7414, 1E7416, 1E741E
    ...
    pub fn detect_iq_limit_maps_by_signature(&self, data: &[u8], detected: &HashSet<u32>) -> Vec<DetectedMap> {
        ...
        for offset in (scan_start..scan_end.saturating_sub(600)).step_by(2) {
            ...
            // the reference tool still lists such a map because it exists structurally.
    ...
    pub fn detect_egr_maps(&self, data: &[u8], detected: &HashSet<u32>) -> Vec<DetectedMap> {
        ...
        //
        // Verified on abcABC_Version 2 (reference file): 10/10 maps at exact
        ...
        for &anchor in &hyst_anchors {
            ...
            'scan: while pos + 4 < anchor {
                ...
                // reference listings; 14-16 on the JA/KN/Superb gear
                ...
                // Jetta1.9, Eos, Golf2l...), absent from every reference list.
        ...
        // Structure based on the reference tool analysis:
        // [00 14] [X axis 40 bytes] [Data 40 bytes] [00 14] [X axis] [Data] ...
        //
        // The reference tool reports the address of the DATA (after the X axis), not the header itself.
        //
        // Known addresses from the reference:
        ...
        for i in 0x180000..data.len().saturating_sub(100) {
            ...
            // unité du groupe (la référence « EGR Hysteresis 5 », 0x1C705A) a un
    ...
    pub fn detect_idle_speed_maps(&self, data: &[u8], detected: &HashSet<u32>) -> Vec<DetectedMap> {
        ...
        for i in 0x10000..data.len().saturating_sub(100) {
            ...
            // Header format: [00 rows][00 cols], the reference tool displays as rows×cols
        ...
        // Based on reference analysis, these follow a specific pattern:
