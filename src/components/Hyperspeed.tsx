...
class Road {
  ...
  createPlane(side: number, width: number, isRoad: boolean) {
    ...
    let uniforms: Record<string, { value: any }> = {
      ...
      uTime: this.uTime,
      uRoadAlpha: { value: isRoad ? 0.25 : 0.25 }
    };
...
const roadBaseFragment = `
  ...
  void main() {
    ...
    gl_FragColor = vec4(color, uRoadAlpha);
    ${THREE.ShaderChunk['fog_fragment']}
  }
`;
...
class App {
  ...
  update(delta: number) {
    ...
    if (typeof this.options.distortion === 'object' && this.options.distortion.getJS) {
      ...
      const cameraMoveFactor = 0.3; // Reduce camera movement (0 = no movement, 1 = full movement)
      this.camera.lookAt(
        new THREE.Vector3(
          this.camera.position.x + distortion.x * cameraMoveFactor,
          this.camera.position.y + distortion.y * cameraMoveFactor,
          this.camera.position.z + distortion.z * cameraMoveFactor
        )
      );
      updateCamera = true;
    }
